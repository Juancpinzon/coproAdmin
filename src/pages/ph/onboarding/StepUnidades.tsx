...
import { OnboardingDraft, UnidadDraft } from './types';
...
export function StepPropietarios({ data, setData, compact }: { data: OnboardingDraft, setData: unknown, compact?: boolean }) {
  ...
  return (
    <div className="anim-fade">
      ...
      {compact ? (
        <div className="space-y-2.5">
          {us.map((u) => <OwnerCard key={u.id} u={u} onChange={(k: string, v: string) => setOwner(u.id, k, v)} />)}
        </div>
      ) : (
        <Card className="divide-y divide-slate-100">
          {us.map((u) => <OwnerRow key={u.id} u={u} onChange={(k: string, v: string) => setOwner(u.id, k, v)} />)}
        </Card>
      )}
    </div>
  );
