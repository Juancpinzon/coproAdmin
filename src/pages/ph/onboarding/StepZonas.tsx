...
export function StepZonas({ data, setData, compact }: { data: OnboardingDraft, setData: any, compact?: boolean }) {
  const zs = data.zonas;
  const setZs = (fn: any) => setData((d: OnboardingDraft) => ({ ...d, zonas: typeof fn === 'function' ? fn(d.zonas) : fn }));
          ...
          {zs.map((z) => (
            <Card key={z.id} className="p-3.5">
              <div className="flex items-start gap-3">
                <span className="w-10 h-10 rounded-xl bg-green-50 text-green-600 flex items-center justify-center shrink-0"><Ico.trees className="w-5 h-5" /></span>
                <div className="flex-1 min-w-0 space-y-2.5">
                  ...
                  <div className={`grid gap-2.5 ${compact ? 'grid-cols-1' : 'grid-cols-3'}`}>
                    <Field label="Capacidad (personas)"><Input className="h-11" inputMode="numeric" value={z.capacidad} placeholder="30" onChange={(e: any) => update(z.id, { ...z, capacidad: e.target.value.replace(/\D/g, '') })} /></Field>
                    <Field label="Apertura"><Input className="h-11" type="time" value={z.apertura} onChange={(e: any) => update(z.id, { ...z, apertura: e.target.value })} /></Field>
                    <Field label="Cierre"><Input className="h-11" type="time" value={z.cierre} onChange={(e: any) => update(z.id, { ...z, cierre: e.target.value })} /></Field>
                  </div>
                </div>
...
function ReviewBlock({ icon: I, title, onEdit, children }: any) {
  return (
    <Card className="p-4">
      <div className="flex items-center gap-2 mb-3">
        <span className="w-7 h-7 rounded-lg bg-blue-50 text-primary flex items-center justify-center shrink-0"><I className="w-4 h-4" /></span>
...
function KV({ k, v, bad }: any) {
  return (
    <div className="flex justify-between gap-3 py-1.5 border-b border-slate-50 last:border-0">
      ...
      <span className={`text-[13px] font-semibold text-right ${bad ? 'text-red-500' : 'text-slate-700'}`}>{v}</span>
    </div>
  );
}

export function StepResumen({ data, goTo }: { data: OnboardingDraft, goTo: (step: number) => void }) {
    ...
    <div className="anim-fade">
      ...
      <div className="space-y-3">
        ...
        <ReviewBlock icon={Ico.grid} title="Unidades" onEdit={() => goTo(3)}>
          ...
          {!coefOk && data.unidades.length > 0 && <p className="text-xs text-red-500 mt-1.5 font-medium">Los coeficientes deben sumar 100%.</p>}
        </ReviewBlock>
...
/* ════════════════════ PANTALLA DE ÉXITO ════════════════════ */
export function SuccessScreen({ data, onRestart, onDashboard }: { data: OnboardingDraft, onRestart: () => void, onDashboard: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-10 px-4 anim-fade">
      <div className="w-20 h-20 rounded-full bg-green-50 flex items-center justify-center mb-5 anim-pop">
        <span className="w-14 h-14 rounded-full bg-green-600 text-white flex items-center justify-center"><Ico.check className="w-8 h-8" /></span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 mt-7 w-full max-w-xs">
        <Button className="flex-1" size="lg" onClick={onDashboard}>Ir al panel</Button>
