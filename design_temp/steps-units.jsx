// steps-units.jsx — Paso 3 (Cargar unidades) y Paso 4 (Asociar propietarios)

const newUnidad = (over = {}) => ({ id: genId(), numero: '', tipo: 'Apartamento', piso: '', torre: '', estrato: '4', coef: '', ...over });

const ESTRATOS = ['1', '2', '3', '4', '5', '6'];

/* ─────────── helpers de coeficiente ─────────── */
const sumCoef = (us) => us.reduce((a, u) => a + (parseFloat(u.coef) || 0), 0);

/* ─────────── Importación masiva (CSV / pegar Excel) ─────────── */
function BulkImport({ onImport, onClose }) {
  ...
  const handleText = (v) => { setText(v); setCount(parse(v).length); };

  const handleFile = (e) => {
    const f = e.target.files?.[0];
    if (!f) return;
    const reader = new FileReader();
    reader.onload = () => { const v = reader.result || ''; setText(v); setCount(parse(v).length); };
    reader.readAsText(f);
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl anim-fade" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            <span className="w-9 h-9 rounded-xl bg-primary-50 text-primary flex items-center justify-center"><Ico.upload className="w-5 h-5" /></span>
            <div>
...
/* ─────────── Tarjeta de unidad (móvil) ─────────── */
function UnidadCard({ u, idx, onChange, onRemove }) {
  const set = (k, v) => onChange({ ...u, [k]: v });
  return (
    <Card className="p-3.5">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[12px] font-bold text-slate-400">UNIDAD {idx + 1}</span>
        <button onClick={onRemove} className="w-9 h-9 -mr-1.5 flex items-center justify-center text-slate-400 hover:text-danger rounded-lg hover:bg-danger-50 transition"><Ico.trash className="w-4 h-4" /></button>
      </div>
...
/* ─────────── Fila de unidad (escritorio) ─────────── */
function UnidadRow({ u, idx, onChange, onRemove }) {
  const set = (k, v) => onChange({ ...u, [k]: v });
  ...
  return (
    <tr className="group">
      ...
      <td className="py-1.5 w-10 text-right">
        <button onClick={onRemove} className="w-8 h-8 inline-flex items-center justify-center text-slate-300 hover:text-danger rounded-lg hover:bg-danger-50 transition opacity-60 group-hover:opacity-100"><Ico.trash className="w-4 h-4" /></button>
      </td>
    </tr>
  );
}

/* ════════════════════ PASO 3 · Cargar unidades ════════════════════ */
function StepUnidades({ data, setData, compact }) {
  const [showImport, setShowImport] = React.useState(false);
  const us = data.unidades;
  const setUs = (fn) => setData((d) => ({ ...d, unidades: typeof fn === 'function' ? fn(d.unidades) : fn }));

  const add = () => setUs((arr) => [...arr, newUnidad()]);
  const update = (id, nu) => setUs((arr) => arr.map((x) => (x.id === id ? nu : x)));
  const remove = (id) => setUs((arr) => arr.filter((x) => x.id !== id));
  const importRows = (rows) => setUs((arr) => [...arr.filter((x) => x.numero || x.torre), ...rows]);
  const distribuir = () => setUs((arr) => arr.length ? arr.map((x) => ({ ...x, coef: (100 / arr.length).toFixed(3) })) : arr);
  ...
  return (
    <div className="anim-fade">
      ...
      <div className={`mb-4 p-3.5 rounded-xl border ${coefOk ? 'bg-success-50 border-success-100' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-[13px] font-semibold text-slate-600">Coeficiente de copropiedad</span>
          <span className={`text-[13px] font-bold tabular-nums ${coefOk ? 'text-success' : 'text-slate-700'}`}>{total.toFixed(2)}% / 100%</span>
        </div>
        <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-300 ${coefOk ? 'bg-success' : total > 100 ? 'bg-danger' : 'bg-primary'}`} style={{ width: Math.min(100, total) + '%' }} />
        </div>
        {!coefOk && us.length > 0 && (
          <p className={`text-xs mt-2 font-medium ${total > 100 ? 'text-danger' : 'text-slate-500'}`}>
      ...
      {us.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {us.map((u, i) => <UnidadCard key={u.id} u={u} idx={i} onChange={(nu) => update(u.id, nu)} onRemove={() => remove(u.id)} />)}
        ...
        <Card className="p-3 overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {us.map((u, i) => <UnidadRow key={u.id} u={u} idx={i} onChange={(nu) => update(u.id, nu)} onRemove={() => remove(u.id)} />)}
            </tbody>
          </table>
...
function StepPropietarios({ data, setData, compact }) {
  const us = data.unidades;
  const setOwner = (id, k, v) => setData((d) => ({
...
function UnitTag({ u, className = '' }) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <span className="w-10 h-10 rounded-xl bg-primary-50 text-primary flex flex-col items-center justify-center shrink-0 leading-none">
        <span className="text-[8px] font-bold uppercase">{TIPO_ABBR[u.tipo]}</span>
  ...
  return (
    <div className="flex items-center gap-3 p-3">
      ...
      <div className="w-56 shrink-0">
        <input className={cell + (emailBad ? ' border-danger ring-2 ring-danger/20' : '')} type="email" value={o.email || ''} placeholder="correo@ejemplo.com" onChange={(e) => onChange('email', e.target.value)} />
      </div>
...
Object.assign(window, { StepUnidades, StepPropietarios, EmptyState, newUnidad, sumCoef, ESTRATOS });
